...
//printf("fn at bts_sync=%d (arfcn=%d, tsc=%d\n", t3, arfcn, tsc);
#if 1
...
static int l1s_tsc_cmd(__unused uint8_t p1, __unused uint8_t p2,
		      __unused uint16_t p3)
{
	uint16_t arfcn;
	uint8_t tsc, tn;
	int t3 = l1s.next_time.t3;
...
#else
	SCHED_ITEM_DT(l1s_nb_cmd, 2, 0, 0),	SCHED_END_FRAME(),
	SCHED_ITEM_DT(l1s_tsc_cmd, 2, 0, 0),	SCHED_END_FRAME(),
	SCHED_ITEM(l1s_nb_resp, -5, 0, 0),	SCHED_END_FRAME(),
#endif
	SCHED_END_SET()
};
